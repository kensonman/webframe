...
<span v-if="data.childs.length>0" class="webframe-navitem" :id="data.id">
   <a class="nav-link dropdown-toggle" href="#" :id="data.id" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
      ...
      <span v-if="data.label">{{ trans(data.label) }}</span>
...
<span v-else class="webframe-navitem" :id="data.id">
   <a :class="data.props.class?data.props.class:'nav-link'" :href="data.props.href?trans(data.props.href):'#'" :target="data.props.target?data.props.target:'_self'" @click.prevent="onclick" @mouseenter="mousein" @mouseleave="mouseout">
      <i v-if="data.icon" :class="'fas '+data.icon"></i> 
      <span v-if="data.label">{{ trans(data.label) }}</span>
      <img v-if="data.image" :src="data.image"/>
   </a>
...
module.exports={
   ...
   ,methods:{
      ...
      ,trans: function(value){
         ...
         val=val.replace(/{\s*url\s*}/g, window.location.pathname.substr(1)+window.location.search);
         return val
      }
   }
};
