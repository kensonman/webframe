...
class Command(BaseCommand):
   ...
   def jasperserver(self, url, **kwargs):
      ...
      headers={'Content-Type': myargs.get('Content-Type', 'application/json')}
      url=url.format(**myargs)
      method=myargs.get('method','put').upper()

      if method=='PUT':
         logger.debug('Putting resources<%s>: %s'%(kwargs['type'], url))
         rep=requests.put(url, auth=auth, data=data, headers=headers)
      elif method=='POST':
         logger.debug('Posting resources<%s>: %s'%(kwargs['type'], url))
         rep=requests.post(url, auth=auth, data=data, headers=headers)
      elif method=='GET':
         logger.debug('Getting resources<%s>: %s'%(kwargs['type'], url))
         rep=requests.get(url, auth=auth, data=data, headers=headers)
      elif method=='DELETE':
         logger.debug('Deleting resources<%s>: %s'%(kwargs['type'], url))
   ...
   def init_role(self):
      ...
      rep=self.jasperserver(url, type='role', method='get')
      if rep.status_code != 200:
         #If NOT exists
         logger.info('Creating reporting role: %s ...'%self.kwargs['role'])
         rep=self.jasperserver(url, data={}, type='role', method='put')
   ...
   def init_user(self):
      ...
      rep=self.jasperserver(url, type='user', method='get', user=user)
      if rep.status_code!=200:
         logger.info('Creating/Updating report user: %s ...'%settings.REPORTING['username'])
         rep=self.jasperserver(url, type='user', method='put', user=user, data={'fullName': 'Deploy by script', 'enabled': True, 'password': pawd, 'rules': [{'name': 'ROLE_USER'}, {'name': self.kwargs['role']}]})
         if 200<=rep.status_code<300:
            logger.info('Role<%s> has been created successfully!'%self.kwargs['role'])
         else:
            logger.warning('Role<%s> cannot been created, status-code: %s'%(self.kwargs['role'], rep.status_code))
   ...
   def import_file(self, filename):
      # Import the xml file
      logger.info('Importing file: {0}'.format(filename))
      root=ET.ElementTree(file=filename).getroot()
      if root.tag=='jdbcDataAdapter':
         #Importing the jdbc
         self.import_ds(root)
      elif root.tag=='{http://jasperreports.sourceforge.net/jasperreports}jasperReport':
   ...
   def import_ds(self, root):
      ...
      url='{rpthost}/rest_v2/resources{prefix}{name}?overwrite=True'
      name=root.find('name').text()
      rep=self.jasperserver(url, type='application/repository.jdbcDataSource+json', method='get')
      if rep.status_code==200:
         self.jasperserver(url, type='application/repository.jdbcDataSource+json', method='delete')
      rep=self.jasperserver(url, type='application/repository.jdbcDataSource+json', method='post', data={
